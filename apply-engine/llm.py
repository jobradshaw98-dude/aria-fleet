...
CAREER_DIR = config.PKG_DIR.parent                  # ~/projects/career
...
def load_facts(job: dict = None, max_chars: int = 200000, recon_brief: str = "") -> str:
    ...
    parts = []
    try:
        # Identity bedrock FIRST — a live session always has this from memory; the headless
        # drafter never did, which is the biggest reason its essays read generic.
        parts.append("# WHO SAM IS (identity bedrock — the unfakeable themes to write FROM; "
                     "style/positioning guidance, NOT a source of factual claims)\n"
                     + NARRATIVE.read_text(encoding="utf-8"))
    except Exception:
        pass
    try:
        parts.append("# VOICE & CRAFT (how Sam writes — style guidance, NOT a source "
                     "of factual claims)\n" + VOICE.read_text(encoding="utf-8"))
    except Exception:
        pass
    try:
        # Capability facts — what Sam can truthfully assert, incl. the AI-native coding framing
        # (he builds/ships production Python via LLM harnesses; frame coding as AI-orchestrated, not
        # unaided fluency). Placed HIGH so it survives the max_chars cap — lets free-text answers
        # ground a coding question in the AI-native frame instead of declining it.
        parts.append("# CAPABILITY FACTS (truthful — incl. how to frame coding/Python: AI-native, "
                     "AI-orchestrated; assert shipped/operated systems, not unaided hand-coding)\n"
    ...
    try:
        ...
        if _rr:
            parts.append("# LEARNED RESUME RULES (durable corrections distilled from past "
                         "edits via /career-learn — style/format guidance, NOT a source of "
                         "factual claims)\n" + _rr)
    except Exception:
        pass
    try:
        parts.append("# RESUME (facts)\n" + RESUME.read_text(encoding="utf-8"))
    except Exception:
        pass
    try:
        # Full claims ledger — the drafter must SEE every vetted claim to pick the strongest,
        # most role-relevant one. The old [:20000] slice combined with the 20k total cap meant
        # it often saw none of these. Fed in full now.
        parts.append("# VETTED CLAIMS — only assert what is supported here\n"
