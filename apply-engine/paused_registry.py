...
CLAUDE.md HARD RULE: `aria-core/data/paused_registry.json` is the SINGLE SOURCE OF TRUTH for kill
switches. Any new UNATTENDED automation must have a pausable control there, and audit/automation
must check it before running. The convergence loop (converge.converge_quality) burns `claude -p`
quota unattended — so it gets a kill switch here (design §8.4 m5).
...
FAIL-SAFE (the brief's hard constraint)
---------------------------------------
...
The registry path is read from `config.ARIA_DATA` (the shared data hub), so it resolves the SAME
file the supervisor + every other ARIA agent reads. Tests point `config.ARIA_DATA` at a throwaway
dir (no network, no real registry mutation).
"""
import json
